...
void updatePlayer() {
  ...
  if (inWater) {
    if (!ceiling) {
      vy -= 0.08;   // buoyancy only if no ceiling
    } else if (vy < 0) {
      vy = 0;       // cancel upward motion
    }
...
void updateWaterFlow() {
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      ...
      // falling column inherits full strength
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = CHUNK_H - 2; y >= 0; y--) {
      ...
      int level = int(currentWater[x][y]);
      if (level <= 0) continue;
      ...
      if (canFlowDown) {
        ...
        continue; // no sideways spreading this tick
      }
      ...
      for (int dir = -1; dir <= 1; dir += 2) {
        ...
        if (isReplaceableBlock(currentTiles[nx][y]) &&
          ...
          currentWater[nx][y] = max(currentWater[nx][y], newLevel);
        }
      }
    }
  }
