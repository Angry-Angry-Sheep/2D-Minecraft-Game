...
void updateGrassSystem() {
  ...
  for (int i = 0; i < 60; i++) {
    ...
    // GRASS → DIRT if blocked above
    if (id == GRASS) {
      if (y > 0) {
        int above = currentTiles[x][y - 1];
        if (!isReplaceableBlock(above)) {
          currentTiles[x][y] = DIRT;
        }
      }
    }

    // DIRT → GRASS if near grass and valid
    else if (id == DIRT) {
      if (y <= 0) continue;

      int above = currentTiles[x][y - 1];
      if (!isReplaceableBlock(above)) continue;
