...
float[][] waterLevel = new float[CHUNK_W][CHUNK_H];
...
void renderWater() {
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      ...
      float level = constrain(waterLevel[x][y] / 8.0, 0, 1);
      if (level <= 0) continue;
...
void updateWaterFlow() {
  ...
  // copy old levels so updates don't instantly feed back
  float[][] old = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      old[x][y] = waterLevel[x][y];
    }
  }

  // pass 1: sources stay sources, flowing recomputed
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      ...
      // recompute flowing level from neighbors
      int best = 0;

      // if water above -> full column (like MC falling streams)
      if (y > 0 && currentTiles[x][y - 1] == WATER) {
        best = 8;
      } else {
        // take best sideways neighbor - 1
        if (x > 0 && currentTiles[x - 1][y] == WATER) best = max(best, int(old[x - 1][y]) - 1);
        if (x < CHUNK_W - 1 && currentTiles[x + 1][y] == WATER) best = max(best, int(old[x + 1][y]) - 1);
      }

      waterLevel[x][y] = max(0, best);
      if (waterLevel[x][y] == 0) currentTiles[x][y] = AIR;
    }
  }

  // pass 2: spread from water tiles into neighbors (down then sides)
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = CHUNK_H - 2; y >= 0; y--) {

      if (currentTiles[x][y] != WATER) continue;

      int level = int(waterLevel[x][y]);
      if (level <= 0) continue;

      // DOWN
      if (isReplaceableBlock(currentTiles[x][y + 1]) && !isPlatformTile(x, y + 1)) {
      ...
      for (int dir = -1; dir <= 1; dir += 2) {
        ...
        if (isReplaceableBlock(currentTiles[nx][y]) && !isPlatformTile(nx, y)) {
          currentTiles[nx][y] = WATER;
          waterLevel[nx][y] = max(waterLevel[nx][y], newLevel);
        }
      }
    }
  }
...
void getWaterSurface(int x, int y, float level, float[] out) {
  ...
  boolean isSource = (waterLevel[x][y] >= 8);
  ...
  if (x > 0 && currentTiles[x - 1][y] == WATER)
    lh = waterLevel[x - 1][y] / 8.0;
  else
    lh = 0;

  if (x < CHUNK_W - 1 && currentTiles[x + 1][y] == WATER)
    rh = waterLevel[x + 1][y] / 8.0;
  else
    rh = 0;
