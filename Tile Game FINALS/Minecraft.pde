...
void draw() {
  ...
  applyCamera();
  if (creativeMode) {
    renderWorldNoLighting();
    renderWater();
      renderPlayer();
    renderTransparentBlocksNoLighting();
  } else {
    renderWorld();
    renderWater();
      renderPlayer();
...
boolean inWater = isPlayerInWater();
boolean ceiling = hasSolidAbovePlayer();
  ...
  if (inWater) {
    ...
    // swimming
    if (upPressed && !ceiling) vy -= 0.6;
    if (downPressed) vy += 0.6;
...
boolean isWaterTile(int tx, int ty) {
  ...
  return currentTiles[tx][ty] == WATER;
}

boolean isLavaTile(int tx, int ty) {
  if (tx < 0 || tx >= CHUNK_W || ty < 0 || ty >= CHUNK_H) return false;
  return currentTiles[tx][ty] == LAVA;
}


boolean isLiquidTile(int tx, int ty) {
  return isWaterTile(tx, ty) || isLavaTile(tx, ty);
}
...
void computeLighting() {
  ...
  // water absorbs skylight
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      if (currentTiles[x][y] == WATER) {
        lightMap[x][y] = min(lightMap[x][y], 120);
      }
    }
  }
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      ...
      if (currentTiles[x][y] == COBBLESTONE) {
        ...
        if (currentTiles[x][y] == LAVA) {
        lightMap[x][y] = max(lightMap[x][y], 280);
       }
    }
  }
...
boolean hasLineOfSight(int x0, int y0, int x1, int y1) {
  ...
  while (true) {
    ...
    int e2 = 2 * dif;
    if (e2 > -dy) { dif -= dy; cx += sx; }
    if (e2 < dx) { dif += dx; cy += sy; }
  }
...
int[][] generateChunk(int cx, int cy) {
  ...
  for (int x = 1; x < CHUNK_W - 1; x++) {
    ...
    float bushNoise  = noise(worldX * 0.15, (surfY + WORLD_SEED) * 0.35);
...
void mousePressed() {
  ...
  if (mouseButton == RIGHT) {
    ...
    if (hotbar[slot] != AIR &&
      ...
      if (placeId == WATER) {
        currentWater[tx][ty] = 8.0;
      }
      else if (placeId == LAVA) {
        currentLava[tx][ty] = 8.0;
      }
...
void getLavaSurface(int x, int y, float level, float[] out) {

  boolean hasAbove = (y > 0 && currentTiles[x][y - 1] == LAVA);
  boolean isSource = (currentLava[x][y] >= 8);

  if (isSource || hasAbove) {
    out[0] = 1.0;
    out[1] = 1.0;
    return;
  }
  ...
  if (x > 0 && currentTiles[x - 1][y] == LAVA)
    lh = currentLava[x - 1][y] / 8.0;

  if (x < CHUNK_W - 1 && currentTiles[x + 1][y] == LAVA)
    rh = currentLava[x + 1][y] / 8.0;
...
float getWaterSurfaceHeight(int tx, int ty) {
  boolean left  = isWaterTile(tx - 1, ty);
  boolean right = isWaterTile(tx + 1, ty);

  if (left && !right) return TILE * 0.75;
  if (!left && right) return TILE * 0.25;
  return TILE * 0.5;
}
...
void renderWater() {
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      ...
      float level = constrain(currentWater[x][y] / 8.0, 0, 1);
      if (level <= 0) continue;
      ...
      int L = lightMap[x][y];
      int airDark = constrain(255 - L, 0, 220);
      fill(0, airDark);
      rect(wx, wy, TILE, TILE);
      
      float[] surf = new float[2];
      getWaterSurface(x, y, level, surf);
      
      float lh = surf[0];
      float rh = surf[1];

      float topL = wy + TILE - lh * TILE;
      float topR = wy + TILE - rh * TILE;

      fill(40, 120, 255, 180);
      beginShape();
      vertex(wx, topL);
      vertex(wx+TILE, topR);
      vertex(wx+TILE, wy+TILE);
      vertex(wx, wy+TILE);
      endShape(CLOSE);

      int darkness = constrain(255 - L, 0, 200);
      fill(0, darkness);
      beginShape();
      vertex(wx, topL);
      vertex(wx+TILE, topR);
...
void updateWaterFlow() {
  ...
  float[][] old = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      old[x][y] = currentWater[x][y];
    }
  }
...
void getWaterSurface(int x, int y, float level, float[] out) {

  boolean hasAbove = (y > 0 && currentTiles[x][y - 1] == WATER);
  boolean isSource = (currentWater[x][y] >= 8);

  float lh = level;
  float rh = level;
  // source blocks are flat
  if (isSource) {
    out[0] = 1.0;
    out[1] = 1.0;
    return;
  ...
  // inherit surface from above
  if (hasAbove) {
    getWaterSurface(x, y - 1, 1.0, out);
    return;
  }

  // compute slope only for topmost visible layer
  if (x > 0 && currentTiles[x - 1][y] == WATER)
    lh = currentWater[x - 1][y] / 8.0;
  else
    lh = 0;

  if (x < CHUNK_W - 1 && currentTiles[x + 1][y] == WATER)
    rh = currentWater[x + 1][y] / 8.0;
  else
    rh = 0;
...
boolean hasSolidAbovePlayer() {
  float headY = py - halfH - 1;
  int txL = int((px - halfW + 2) / TILE);
  int txR = int((px + halfW - 2) / TILE);
  int ty  = int(headY / TILE);
...
void applyWaterCurrent() {
  ...
  if (currentTiles[tx][ty] != WATER) return;

  // Falling water pushes down
  if (ty > 0 && currentTiles[tx][ty - 1] == WATER && currentWater[tx][ty] >= 8) {
    vy += 0.25;
  }

  // Horizontal flow
  float left  = (tx > 0 && currentTiles[tx - 1][ty] == WATER) ? currentWater[tx - 1][ty] : 0;
  float right = (tx < CHUNK_W - 1 && currentTiles[tx + 1][ty] == WATER) ? currentWater[tx + 1][ty] : 0;

  float flow = (left-right) * 0.12;
  vx += flow;
}

float[][] cloneWater(float[][] src) {
  float[][] out = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    arrayCopy(src[x], out[x]);
  }
  return out;
}

void saveCurrentChunkState() {
  String key = currentChunkX + "," + currentChunkY;
  savedChunks.put(key, currentTiles);
  savedWater.put(key, cloneWater(currentWater));
  savedLava.put(key, cloneLava(currentLava));
}

float[][] loadLavaChunk(int cx, int cy, int[][] tiles) {
  String key = cx + "," + cy;

  if (savedLava.containsKey(key)) {
    return savedLava.get(key);
  }

  float[][] l = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      if (tiles[x][y] == LAVA) l[x][y] = 8.0;
    }
  }
  savedLava.put(key, l);
  return l;
...
float[][] loadWaterChunk(int cx, int cy, int[][] tiles) {
  String key = cx + "," + cy;

  if (savedWater.containsKey(key)) {
    return savedWater.get(key);
  }

  // initialize water tiles
  float[][] w = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      if (tiles[x][y] == WATER) w[x][y] = 8.0;  // default water value
    }
  }
  savedWater.put(key, w);
  return w;
}

//draws the item texture for water and lava
void drawItemIcon(int id, float x, float y, float size) {
  if (id == WATER) {
    noStroke();
    fill(40, 120, 255);
    rectMode(CENTER);
...
void renderTransparentBlocksNoLighting() {
  rectMode(CORNER);
  imageMode(CORNER);
...
void renderLava() {
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      ...
      if (currentTiles[x][y] != LAVA) continue;

      float level = constrain(currentLava[x][y] / 8.0, 0, 1);
      if (level <= 0) continue;
      ...
      int L = lightMap[x][y];
      int dark = constrain(255 - L, 0, 180);
      ...
      float[] surf = new float[2];
      getLavaSurface(x, y, level, surf);
      
      float lh = surf[0];
      float rh = surf[1];
      
      float topL = wy + TILE * (1 - lh);
      float topR = wy + TILE * (1 - rh);
...
void updateLavaFlow() {
  ...
  if (frameCount % 4 != 0) return; // 4x slower than water

  float[][] old = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = 0; y < CHUNK_H; y++) {
      old[x][y] = currentLava[x][y];
    }
  }
  ...
  for (int x = 0; x < CHUNK_W; x++) {
    for (int y = CHUNK_H - 1; y >= 0; y--) {
      ...
      int spread = level - 2; // lava spreads less
      ...
      for (int dir = -1; dir <= 1; dir += 2) {
        ...
        if (isReplaceableBlock(currentTiles[nx][y])) {
          currentTiles[nx][y] = LAVA;
          currentLava[nx][y] = max(currentLava[nx][y], spread);
        }
      }
    }
  }
...
void saveWorld() {
  ...
  player.setFloat("x", px);
  player.setFloat("y", py);
  root.setJSONObject("player", player);
  ...
  root.setJSONObject("chunks", chunks);

  String filename = "world_" + year() + "_" + month() + "_" + day() + "_" +
                    hour() + "_" + minute() + "_" + second() + ".json";

  saveJSONObject(root, dataPath(WORLD_FOLDER + "/" + filename));
  println("World saved:", filename);
}

JSONArray int2DToJSON(int[][] src) {
...
JSONArray float2DToJSON(float[][] src) {
  JSONArray arr = new JSONArray();
  for (int x = 0; x < CHUNK_W; x++) {
    JSONArray col = new JSONArray();
    for (int y = 0; y < CHUNK_H; y++) {
...
void loadWorld() {
  // This does not work, it can't find the world files
  droppedItems.clear();
  chestEntities.clear();
  openChest = null;
  chestOpen = false;
  ...
  File worldDir = new File(dataPath(WORLD_FOLDER));
  println("WORLD DIR:", worldDir.getAbsolutePath());
  ...
  // ensure deterministic "first file"
  Arrays.sort(files);
  File worldFile = files[0];

  println("Loading world:", worldFile.getName());

  // load JSON
  JSONObject root = loadJSONObject(worldFile);
  ...
  px = player.getFloat("x");
  py = player.getFloat("y");

  // sync chunk coordinates
  currentChunkX = floor(px / (CHUNK_W * TILE));
  currentChunkY = floor(py / (CHUNK_H * TILE));

  // convert to local chunk xy
  px -= currentChunkX * CHUNK_W * TILE;
  py -= currentChunkY * CHUNK_H * TILE;

  // load chunks
  JSONObject chunks = root.getJSONObject("chunks");
  for (Object k : chunks.keys()) {
    String key = (String) k;
    JSONObject c = chunks.getJSONObject(key);

    savedChunks.put(key, jsonToInt2D(c.getJSONArray("tiles")));
    savedWater.put(key, jsonToFloat2D(c.getJSONArray("water")));
    savedLava.put (key, jsonToFloat2D(c.getJSONArray("lava")));
  }
...
int[][] jsonToInt2D(JSONArray arr) {
  int[][] out = new int[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    JSONArray col = arr.getJSONArray(x);
    for (int y = 0; y < CHUNK_H; y++) {
      out[x][y] = col.getInt(y);
    }
  }
  return out;
}

float[][] jsonToFloat2D(JSONArray arr) {
  float[][] out = new float[CHUNK_W][CHUNK_H];
  for (int x = 0; x < CHUNK_W; x++) {
    JSONArray col = arr.getJSONArray(x);
    for (int y = 0; y < CHUNK_H; y++) {
      out[x][y] = col.getFloat(y);
    }
  }
  return out;
}
