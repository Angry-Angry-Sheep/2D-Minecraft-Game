...
if (mouseButton == LEFT) {
  if (isBreakableTile(selTileX, selTileY)) {
    int id = currentTiles[selTileX][selTileY];

    if (id != AIR) {
      spawnItemDrop(selTileX, selTileY, id, 1);
    }
  ...
  if (mouseButton == RIGHT) {
    ...
    int slot = selectedSlot;
    int targetId = currentTiles[selTileX][selTileY];

    if (hotbar[slot] != AIR &&
        hotbarCount[slot] > 0 &&
        isReplaceableBlock(targetId)) {

      currentTiles[selTileX][selTileY] = hotbar[slot];
      hotbarCount[slot]--;
...
boolean hasSupportBelow(int x, int y) {
  if (y >= CHUNK_H - 1) return true; // bottom of world
...
void updateGrassSystem() {
  ...
  for (int i = 0; i < 60; i++) {
    ...
    if ((id == GRASS || id == BUSH) && !hasSupportBelow(x, y)) {
